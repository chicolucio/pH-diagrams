...
class TestGenericDiproticAcid:
    ...
    def test_generic_diprotic_acid_alphas(self):
        assert np.allclose(self.generic_diprotic.alpha,
                           np.loadtxt(f'{FOLDER}gen_diprotic_acid_alphas.csv', delimiter=',', unpack=True))

    def test_generic_diprotic_acid_logc(self):
        assert np.allclose(self.generic_diprotic.log_concentrations,
                           np.loadtxt(f'{FOLDER}gen_diprotic_acid_logc.csv', delimiter=',', unpack=True))
...
class TestGenericTriproticAcid:
    ...
    def test_generic_triprotic_acid_alphas(self):
        assert np.allclose(self.generic_triprotic.alpha,
                           np.loadtxt(f'{FOLDER}gen_triprotic_acid_alphas.csv', delimiter=',', unpack=True))

    def test_generic_triprotic_acid_logc(self):
        assert np.allclose(self.generic_triprotic.log_concentrations,
                           np.loadtxt(f'{FOLDER}gen_triprotic_acid_logc.csv', delimiter=',', unpack=True))
...
class TestCrIII:
    ...
    def test_CrIII_alphas(self):
        assert np.allclose(self.CrIII.alpha,
                           np.loadtxt(f'{FOLDER}CrIII_hydrolysis_alphas.csv', delimiter=',', unpack=True))

    def test_CrIII_logc(self):
        assert np.allclose(self.CrIII.log_concentrations,
                           np.loadtxt(f'{FOLDER}CrIII_hydrolysis_logc.csv', delimiter=',', unpack=True))
