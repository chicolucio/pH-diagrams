...
class TestAceticAcid:
    ...
    def test_acetic_acid_alphas(self):
        assert np.allclose(self.acetic_acid.alpha,
                           np.loadtxt(f'{FOLDER}acetic_acid_alphas.csv',
                                      delimiter=',', unpack=True))

    def test_acetic_acid_logc(self):
        assert np.allclose(self.acetic_acid.log_concentrations,
                           np.loadtxt(f'{FOLDER}acetic_acid_logc.csv',
                                      delimiter=',', unpack=True))
...
class TestFumaricAcid:
    ...
    def test_fumaric_acid_alphas(self):
        assert np.allclose(self.fumaric_acid.alpha,
                           np.loadtxt(f'{FOLDER}fumaric_acid_alphas.csv',
                                      delimiter=',', unpack=True))

    def test_fumaric_acid_logc(self):
        assert np.allclose(self.fumaric_acid.log_concentrations,
                           np.loadtxt(f'{FOLDER}fumaric_acid_logc.csv',
                                      delimiter=',', unpack=True))
...
class TestGenericDiproticAcid:
    ...
    def test_generic_diprotic_acid_alphas(self):
        assert np.allclose(self.generic_diprotic.alpha,
                           np.loadtxt(f'{FOLDER}gen_diprotic_acid_alphas.csv',
                                      delimiter=',', unpack=True))

    def test_generic_diprotic_acid_logc(self):
        assert np.allclose(self.generic_diprotic.log_concentrations,
                           np.loadtxt(f'{FOLDER}gen_diprotic_acid_logc.csv',
                                      delimiter=',', unpack=True))
...
class TestGenericTriproticAcid:
    ...
    def test_generic_triprotic_acid_alphas(self):
        assert np.allclose(self.generic_triprotic.alpha,
                           np.loadtxt(f'{FOLDER}gen_triprotic_acid_alphas.csv',
                                      delimiter=',', unpack=True))

    def test_generic_triprotic_acid_logc(self):
        assert np.allclose(self.generic_triprotic.log_concentrations,
                           np.loadtxt(f'{FOLDER}gen_triprotic_acid_logc.csv',
                                      delimiter=',', unpack=True))
...
class TestTyrosine:
    ...
    def test_tyrosine_alphas(self):
        assert np.allclose(self.tyrosine.alpha,
                           np.loadtxt(f'{FOLDER}tyrosine_alphas.csv',
                                      delimiter=',', unpack=True))

    def test_tyrosine_logc(self):
        assert np.allclose(self.tyrosine.log_concentrations,
                           np.loadtxt(f'{FOLDER}tyrosine_logc.csv',
                                      delimiter=',', unpack=True))
